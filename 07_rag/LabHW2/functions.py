# functions.py
# LabHW2 — Function calling helpers (self-contained copy)
# Adapted from 08_function_calling/functions.py for this assignment folder.
# Pairs with course function-calling materials / functions.R
# Tim Fraser
...
# pip install requests pandas tabulate  # tabulate: optional dependency for to_markdown

## 0.2 Configuration #################################

# Default model and Ollama connection
DEFAULT_MODEL = "smollm2:1.7b"
# Override if Ollama runs on another host/port: export OLLAMA_HOST=http://127.0.0.1:11434
...
def _globals_for_tool_dispatch():
    ...
    inspect.currentframe().f_back points at agent_run() here, so we must walk up
    until we leave functions.py — otherwise every tool lookup misses and Ollama
...
def _recover_tool_output_from_text_content(content, tools, caller_globals):
    """
    Some models (e.g. llama3.2) return a JSON-shaped tool description in message.content
    instead of populating message.tool_calls. If it matches a registered tool name, run it.

    Returns
    -------
    tuple[str, Any] | None
        (tool_name, return_value) or None
    """
...
def agent(
    ...
    tool_choice : str, dict, or None
        When using tools: passed to Ollama as tool_choice (e.g. "required" or a
        function dict). None omits the field (older Ollama). Default "required".
    ...
    # Model put a fake JSON tool call in .content instead of native tool_calls (common w/ llama3.2)
    if tools:
        caller_globals = _globals_for_tool_dispatch()
...
def agent_run(
    ...
    tool_choice : str, dict, or None
        Ollama tool_choice when tools is set (default "required"). None to omit.
